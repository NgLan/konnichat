...
int db_send_friend_request(int sender_id, int target_id)
{
    ...
    if (res && mysql_num_rows(res) > 0) { mysql_free_result(res); return -1; } // Đã là bạn
    ...
    if (res && mysql_num_rows(res) > 0) { mysql_free_result(res); return -2; } // Đang chờ
    if (res) mysql_free_result(res);
...
int db_respond_friend_request(int request_id, int current_user_id, int is_accepted, int *sender_id_out)
{
    ...
    if (res) {
        ...
        if (row) { sender_id = atoi(row[0]); receiver_id = atoi(row[1]); }
        mysql_free_result(res);
    }

    if (receiver_id != current_user_id) return 0; // Bảo mật: Không được duyệt hộ người khác
