...
int recv_all(int sock, void *buffer, int size) {
    ...
    while (total_received < size) {
        ...
        if (received <= 0) return received; // Lỗi hoặc ngắt kết nối
    ...
    while (1) {
        ...
        switch (header.command_type) {
            case CMD_REGISTER: {
                ...
                int success = db_register_user(data->username, data->password);
            ...
            case CMD_LOGIN: {
                ...
                int userId = db_check_login(data->username, data->password);
